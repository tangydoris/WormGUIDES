...
public class StoryFileUtil {
	...
	public static File saveToCSVFile(Story story, File file, int offset) throws IOException{
		...
		try  
		{
		    FileWriter fstream = new FileWriter(file, false); // false means do not append
		    out = new BufferedWriter(fstream);
		    
		    // do writing
		    // headers
		    out.append(NAME).append(CS)
		    	.append(CONTENTS).append(CS)
		    	.append(DISPLAY).append(CS)
		    	.append(ATTACHMENT).append(CS)
		    	.append(LOCATION).append(CS)
		    	.append(CELLS).append(CS)
		    	.append(MARKER).append(CS)
		    	.append(SOURCE).append(CS)
		    	.append(RESOURCE).append(CS)
		    	.append(START).append(CS)
		    	.append(END).append(CS)
		    	.append(COMMENTS).append(CS)
		    	.append(AUTHOR).append(CS)
		    	.append(DATE).append(CS)
		    	.append(COLOR);
		    out.append(BR);
		    ...
		    for (Note note : story.getNotes()) {
		    	...
		    	// if time is no specified, do not use the Integer.MIN_VALUE value
		    	// leave it blank
