...
public class Quaternion {
	...
	/**
	 * 	 * Based on the angle of rotation, this method computes the local_rotation quaternion and then updates
	 * 'this' quaternion
	 * 
	 * Local rotation quaternion values:
	 * 	w = cos(angOfRotation/2)
	 *  x = axis.x * sin(angOfRotation/2)
	 *  y = axis.y * sin(angOfRotation/2)
	 *  z = axis.z * sin(angOfRotation/2)
	 * 
	 * @param angOfRotation - the angle of rotation
	 * @param axisX - the x axis of the rotation direction
	...
	/**
	 * Performs the quaternion update:
	...
	public ArrayList<Double> toEulerRotation() {
		 ...
		 * 
		 * North pole: x*y + z*w = 0.5
		 * --> which gives
		 * 		heading = 2 * atan2(x, w)
		 * 		bank = 0
		 * 
		 * South pole: x*y + z*w = -0.5
		 * --> which gives
		 * 		heading = -2 * atan2(x, w)
		 * 		bank = 0
		 */
		//TODO check if it should be greater or less than NORTH_POLE or SOUTH_POLE
		double f = (x*y) + (z*w);
		if (f > NORTH_POLE) {
...
//			bank = 0;
//		} else {
//			heading = Math.atan2(2*this.getY()*this.getW() - 2*this.getX()*this.getZ(), sqx-sqy-sqz+sqw);
//			attitude = Math.asin(2*(test/unit));
//			bank = Math.atan2(2*this.getX()*this.getW() - 2*this.getY()*this.getZ(), -sqx+sqy-sqz+sqw);
//		}
//		
